    ...
    Module tests("State", [](Describe & describe) {
        ...
        describe.describe("when constructed with an existing state", [](Describe & describe) {
            describe.it("should increment the sequence", []() {
                ...
                TEST_ASSERT_EQUAL(0, state1.reduxSequence);
                TEST_ASSERT_EQUAL(1, state2.reduxSequence);
                TEST_ASSERT_EQUAL(2, state3.reduxSequence);
                TEST_ASSERT_EQUAL(1, state4.reduxSequence);
            });
        });
